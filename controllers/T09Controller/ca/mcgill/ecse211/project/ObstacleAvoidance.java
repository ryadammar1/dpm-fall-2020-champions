...
public class ObstacleAvoidance implements Runnable {
	...
	// These arrays are used to avoid creating new ones at each iteration.
  	...
  	/** Main method run in a separate thread.*/
  	@Override
  	public void run() { 
  	...
  	* @param direction direction in which the correction is done. (1:clockwise, 2:counter-clockwise)
  	*/
  	private void goAround(int direction) {
  		int coefficient = 0;
  		if (direction == 1) {
  			coefficient = -1;
  		}
